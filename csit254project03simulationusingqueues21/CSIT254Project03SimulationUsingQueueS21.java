 ...
 *
 * @author Michael Main, Stephen Brower, and Andy Fordjour
 */
    ...
    public static void main(String[] args) {
        ...
        for (currentSecond = 0; currentSecond < MAX_TIME_FOR_SIMULATION; currentSecond++) {
            ...
            if (Math.random() < PROBABILITY_OF_ARRIVAL) {
                ...
                if (newCustomer.getInitialWorkToDo() <= 60) {
                    ...
                    for (int serverNum = 0; serverNum < regularServer.length; serverNum++) {
                        if (regularServer[serverNum].isFree()) {
                            selectedServer = serverNum;
                        } else {
                            if (regularServer[serverNum].queueSize() < regularServer[selectedServer].queueSize()) {
                                //  selectedServer +=
                                selectedServer = serverNum;
                            }
                        }
                ...
                /* add code here */
                // for (int serverNum = 0; serverNum < expressServer.length; serverNum++) {
                ...
                if (currentSecond == (MAX_TIME_FOR_SIMULATION - 1)) {
                    ...
                    /* add code here */
                    //for (int serverNum = 0; serverNum < expressServer.length; serverNum++) {
                    System.out.println("\t\t\t\t\t\tExpress Server("
                            + "): number of customers stuck on line: " + expressServer.queueSize());
                }
            ...
            System.out.printf("Average Wait Time of ALL customers Served: %,.1f\n",
                    ((double) totalWaitingTime / (totalRegular)));  // include with totalRegular the expressServers customers served
            /* add code here */
            int totalExpress = 0;
            totalExpress += expressServer.getCustomersServed();

            System.out.printf("Average Wait Time of ALL customers Served: %,.1f\n",
                    ((double) totalWaitingTime / (totalExpress)));
            ...
            /* add code here */
            //for (int serverNum = 0; serverNum < expressServer.length; serverNum++) {
            System.out.println("Regular Server : number of customers stuck on line: "
